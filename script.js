...
const TELEGRAM_ORDER_URL = "https://t.me/Delta_gameshop";
...
/* Remember where user opened Cart from (Shop/Plan) + their scroll position */
let CART_ENTRY_VIEW = null;
let CART_ENTRY_SCROLL_Y = 0;
...
/* pending selections on Plan page only (NOT committed yet) */
...
/* ===== CART (committed) qty helpers ===== */
function getCartQty(pid, planId){
  const k = cartKey(pid, planId);
  return CART.get(k)?.qty || 0;
}

function setCartQty(pid, planId, qty){
  const k = cartKey(pid, planId);
  if (qty <= 0){
    CART.delete(k);
  } else {
...
/* ===== PENDING (plan page only) qty helpers ===== */
function getPendingQty(pid, planId){
  const k = cartKey(pid, planId);
  return PENDING.get(k) || 0;
}

function setPendingQty(pid, planId, qty){
  const k = cartKey(pid, planId);
  const next = Math.max(0, Number(qty) || 0);
  if (next <= 0) PENDING.delete(k);
  else PENDING.set(k, next);
}

/* Clear all pending rows for a product */
function clearPendingForProduct(pid){
  for (const k of Array.from(PENDING.keys())){
    if (k.startsWith(pid + "::")) PENDING.delete(k);
  }
}
...
function isShopFlowView(v){
  // Cart bar visible only while browsing/adding (NOT on Cart/Checkout pages)
  return v === shopView || v === planView;
}
...
cartbarCheckoutBtn && cartbarCheckoutBtn.addEventListener("click", ()=>{
  // ✅ Save where user came from + exact scroll position
  if (ACTIVE_VIEW === shopView || ACTIVE_VIEW === planView){
    CART_ENTRY_VIEW = ACTIVE_VIEW;
    CART_ENTRY_SCROLL_Y = window.scrollY || 0;
  } else {
    CART_ENTRY_VIEW = null;
    CART_ENTRY_SCROLL_Y = 0;
  }

  renderCart();
  showView(cartView, { resetScroll:true }); // cart page opens at top (clean)
});
...
function renderPlanGroups(product){
  ...
  planGroupsEl.innerHTML = groups.map(g => `
    <div class="plan-group">
      ...
      <div class="plan-rows">
        ${g.plans.map(pl => {
          const qty = getPendingQty(product.id, pl.id);
          return `
  ...
  planGroupsEl.querySelectorAll(".plan-row").forEach(row=>{
    row.addEventListener("click", (e)=>{
      ...
      const current = getPendingQty(currentPlanProduct.id, planId);

      const next = act === "plus" ? current + 1 : current - 1;
      setPendingQty(currentPlanProduct.id, planId, next);
...
/* Add to Cart (commit pending selections, reset plan to 0, stay on plan page) */
goCartBtn && goCartBtn.addEventListener("click", ()=>{
  if (!currentPlanProduct) return;

  // Commit pending selections for this product into CART
  currentPlanProduct.plans.forEach(pl=>{
    const addQty = getPendingQty(currentPlanProduct.id, pl.id);
    if (addQty <= 0) return;

    const existing = getCartQty(currentPlanProduct.id, pl.id);
    setCartQty(currentPlanProduct.id, pl.id, existing + addQty);
  });

  // Reset pending selections for this product back to 0
  clearPendingForProduct(currentPlanProduct.id);

  // Re-render plan so all numbers show 0 again
  renderPlanGroups(currentPlanProduct);

  // Update cart UI (global bar + mini count)
  updateCartMini();
  updateCartBar();
});

/* Cart back -> return to where Checkout was clicked (Shop/Plan) with scroll restore */
cartBackBtn && cartBackBtn.addEventListener("click", ()=>{
  // Priority: go back to the exact entry view (shop/plan) if known
  if (CART_ENTRY_VIEW === shopView){
    showView(shopView, { resetScroll:false });
    setNavActive(shopBtn);
    restoreScrollY(CART_ENTRY_SCROLL_Y);
    return;
  }

  if (CART_ENTRY_VIEW === planView && currentPlanProduct){
    // Make sure plan UI is correct when returning
    renderPlanGroups(currentPlanProduct);
    updateCartMini();
    showView(planView, { resetScroll:false });
    restoreScrollY(CART_ENTRY_SCROLL_Y);
    return;
  }

  // Fallback behavior (if entry unknown)
...
function renderCart(){
  ...
  cartItemsEl.querySelectorAll(".cart-item").forEach(row=>{
    row.addEventListener("click", (e)=>{
      ...
      const act = btn.getAttribute("data-act");
      const cur = getCartQty(pid, planId);
      const next = act === "plus" ? cur + 1 : cur - 1;
      setCartQty(pid, planId, next);
...
function renderCheckout(){
  ...
  // Telegram deep link with prefilled order message
  if (checkoutTelegram){
    const msg = buildTelegramMessage();
